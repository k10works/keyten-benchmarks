...
STATS_DIR=""
ENGINES="kdb,sql,duckdb,polars,pykx,pandas,rayforce,keyten"
...
Options:
  ...
  -e, --engines      Comma-separated list of engines to test (default: "kdb,sql,duckdb,polars,pykx,pandas,rayforce,keyten")
...
function execute_queries () {
    ...
    for s in "${THREAD_NRS[@]}"; do
        ...
        if engine_enabled keyten; then
            # Keyten scans the same parquet dataset as the other engines
            # through the shared python queryrunner via the keyten python
            # bindings (KEYTEN_PY_PKG, default sibling checkout).
            # KEYTEN_WORKERS=0 maps to the engine's auto worker count.
            KEYTEN_PY_PKG="${KEYTEN_PY_PKG:-../keyten-py}"
            KEYTEN_WORKERS=${s} $(get_numa_config) uv run --with ${KEYTEN_PY_PKG} pysrc/queryrunner/main.py ${COMMONPARAMS} $(query_output_param keyten) -date $DATE -db ${DB_DIR}/parquet/rowgroup -engine keyten -sortcols "sym,time" -queryfile ./artifacts/queries/inmemory/keyten.psv -result ${RESULT_DIR}/keyten_${s}Threads.psv
            add_nickname ${RESULT_DIR}/keyten_${s}Threads.psv "keyten"
        fi
    done
}
