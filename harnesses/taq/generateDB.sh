...
elif [[ ${DATAFORMAT} == "keyten" ]]; then
  echo "Generating keyten native dataset..."
  KEYTEN_RUNNER="${KEYTEN_RUNNER:-${script_dir}/src/keyten/runner/target/release/keyten-runner}"
  if [[ ! -x "${KEYTEN_RUNNER}" ]]; then
    echo "Building keyten-runner..."
    cargo build --release --manifest-path "${script_dir}/src/keyten/runner/Cargo.toml"
  fi
  "${KEYTEN_RUNNER}" --prepare --csvdir "${CSVDIR}" --dst "${DST}"
else
